# 🚀 Infrastructure - Установка и запуск на Windows
...
3. Убедитесь, что WSL 2 backend включен (рекомендуется)
...
**Подключение через psql (если установлен):**
```powershell
...
5. На вкладке **Connection** укажите:
   - Host: `host.docker.internal` (для Windows)
   - Port: `5432`
   - Database: `attendance`
...
Get-Content backup.sql | docker exec -i attendance-postgres psql -U attendance_user attendance
```
...
### Ошибка: "Drive sharing seems blocked by a firewall"

1. Откройте **Docker Desktop** → **Settings** → **Resources** → **File Sharing**
2. Добавьте диск с проектом (например, `C:`)
...
└── SETUP_WINDOWS.md          # Эта инструкция
```
...
1. **Регулярные backup'ы:**
   ...
   docker exec attendance-postgres pg_dump -U attendance_user attendance | Out-File "backup_$date.sql"
   ```
...
5. **Добавьте Docker volumes в исключения антивируса** для ускорения работы
...
### WSL 2 vs Hyper-V
- **WSL 2** (рекомендуется) - лучшая производительность
- **Hyper-V** - старый backend, медленнее

### Пути к файлам
Windows использует обратные слэши `\`, но в Docker Compose используйте прямые `/`:
```yaml
volumes:
